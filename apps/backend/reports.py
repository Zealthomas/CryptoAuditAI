...
🔒 UPGRADED: Now includes mixer interaction statistics and analysis
"""
...
class ReportGenerator:
        ...
        async with AsyncSessionLocal() as session:
            ...
            for tx in transactions:
                if tx.compliance_flags and "MIXER_INTERACTION" in tx.compliance_flags:
                    mixer_count += 1
                    mixer_transactions.append(tx.id)
                    mixer_volume += tx.amount
            ...
            return {
                ...
                "sanctioned_mixer_count": sanctioned_mixer_count,
                "mixer_transactions": mixer_transactions,
                "mixer_names": list(mixer_names),
                "mixer_volume": mixer_volume,
                "mixer_percentage": (mixer_count / max(total_transactions, 1)) * 100,
                # Other stats
                "sources": sources,
                "assets": assets,
    ...
    async def generate_executive_summary(self, stats: Dict[str, Any], compliance: Dict[str, Any]) -> str:
        ...
        🔒 UPGRADED: Includes mixer interaction context
        """
        mixer_alert = ""
        if stats['mixer_count'] > 0:
            mixer_alert = f"\n🚨 CRITICAL: {stats['mixer_count']} mixer interaction(s) detected"
            if stats['sanctioned_mixer_count'] > 0:
                mixer_alert += f" ({stats['sanctioned_mixer_count']} with OFAC sanctioned entities)"
...
- Mixer Transactions: {stats['mixer_count']} ({stats['mixer_percentage']:.1f}% of total)
- Sanctioned Mixers: {stats['sanctioned_mixer_count']}
- Mixer Services Detected: {', '.join(stats['mixer_names']) if stats['mixer_names'] else 'None'}
- Total Mixer Volume: ${stats['mixer_volume']:,.2f}
...
2. Key findings and concerns (especially mixer interactions)
3. Compliance status and sanctions violations
4. Immediate action items
5. Money laundering risk indicators

Write in professional, clear language suitable for executives and regulators. 
CRITICAL: Emphasize any mixer interactions or sanctioned entity exposure."""
    ...
    async def generate_detailed_analysis(self, user_id: int, stats: Dict[str, Any]) -> str:
        ...
        🔒 UPGRADED: Includes mixer interaction analysis
        """
        async with AsyncSessionLocal() as session:
            ...
            # 🔒 Get mixer transactions
            mixer_tx_result = await session.execute(
                select(Transaction)
                .where(Transaction.user_id == user_id)
                .where(Transaction.id.in_(stats['mixer_transactions'][:5]))
            )
            mixer_transactions_list = mixer_tx_result.scalars().all()
            ...
            # 🔒 Mixer transaction details
            mixer_details = []
            for tx in mixer_transactions_list:
                mixer_info = None
                if tx.to_address:
                    mixer_info = get_mixer_info(tx.to_address)
                if not mixer_info and tx.from_address:
                ...
                mixer_name = mixer_info['name'] if mixer_info else "Unknown Mixer"
                sanctioned_flag = " [SANCTIONED]" if mixer_info and mixer_info['sanctioned'] else ""
                mixer_details.append(
                    f"- ${tx.amount} {tx.asset} via {mixer_name}{sanctioned_flag} "
                    f"on {tx.timestamp.strftime('%Y-%m-%d')}"
                )

            context = f"""
Detailed Transaction Pattern Analysis:

🚨 Mixer Interaction Analysis ({stats['mixer_count']} total):
{chr(10).join(mixer_details[:5]) if mixer_details else "No mixer transactions detected"}
...
- Mixer interaction rate: {stats['mixer_percentage']:.1f}%
"""
...
3. Potential money laundering indicators (especially mixer usage)
4. Regulatory compliance concerns and sanctions violations
5. Specific recommendations for risk mitigation

CRITICAL: Provide detailed analysis of any mixer interactions as these indicate high money laundering risk.
Be specific and technical while remaining actionable."""
    ...
    async def generate_recommendations(self, stats: Dict[str, Any], compliance: Dict[str, Any]) -> List[str]:
        ...
        🔒 UPGRADED: Prioritizes mixer-related recommendations
        """
...
- Compliance Score: {compliance['compliance_score']}/100
- Unresolved Alerts: {stats['unresolved_alerts']}
- 🚨 Mixer Interactions: {stats['mixer_count']}
- ⚠️  Sanctioned Mixer Interactions: {stats['sanctioned_mixer_count']}
"""
        recommendations_prompt = "Provide 5 specific, actionable recommendations to improve security and compliance posture. Prioritize mixer-related issues."
        ...
        # 🔒 Add mixer-specific recommendations if detected
        if stats['mixer_count'] > 0:
            mixer_recs = [
                "🚨 URGENT: Investigate all mixer interactions for money laundering indicators",
                "Document business purpose for all mixer transactions immediately"
            ]
            if stats['sanctioned_mixer_count'] > 0:
                mixer_recs.insert(0, "⚠️  CRITICAL: Report OFAC sanctioned entity interactions to legal/compliance immediately")
            recommendations = mixer_recs + recommendations[:3]
    ...
    def _create_pdf_report(self, report_data: Dict[str, Any], filepath: str):
        ...
        🔒 UPGRADED: Includes mixer interaction section
        """
        ...
        # 🔒 MIXER ALERT (if detected)
        if report_data['stats']['mixer_count'] > 0:
            mixer_alert_style = ParagraphStyle(
                'MixerAlert',
            ...
            story.append(Paragraph(
                f"🚨 CRITICAL ALERT: {report_data['stats']['mixer_count']} Mixer Interaction(s) Detected",
                mixer_alert_style
            ))
            if report_data['stats']['sanctioned_mixer_count'] > 0:
                story.append(Paragraph(
                    f"⚠️  WARNING: {report_data['stats']['sanctioned_mixer_count']} OFAC Sanctioned Entity Interaction(s)",
                    mixer_alert_style
                ))
        ...
        stats_data = [
            ...
            # 🔒 MIXER STATISTICS
            ['🚨 Mixer Interactions', str(report_data['stats']['mixer_count'])],
            ['⚠️  Sanctioned Mixers', str(report_data['stats']['sanctioned_mixer_count'])],
            ['Mixer Volume', f"${report_data['stats']['mixer_volume']:,.2f}"],
        ]
        ...
        # 🔒 Mixer Interaction Details (if any)
        if report_data['stats']['mixer_count'] > 0:
            story.append(Paragraph("🚨 Mixer Interaction Analysis", styles['Heading2']))
            mixer_text = f"""
            <b>Total Mixer Interactions:</b> {report_data['stats']['mixer_count']}<br/>
            <b>Sanctioned Mixers:</b> {report_data['stats']['sanctioned_mixer_count']}<br/>
            <b>Mixer Services Detected:</b> {', '.join(report_data['stats']['mixer_names'])}<br/>
            <b>Total Volume:</b> ${report_data['stats']['mixer_volume']:,.2f}<br/>
            <b>Risk Assessment:</b> CRITICAL - Immediate investigation required<br/>
            <br/>
            Mixer interactions indicate potential money laundering activity and must be investigated immediately.
            Transactions with OFAC sanctioned mixers must be reported to authorities.
            """
            story.append(Paragraph(mixer_text, styles['Normal']))
            story.append(Spacer(1, 15))
    ...
    async def generate_comprehensive_report(
        ...
        🔒 UPGRADED: Includes comprehensive mixer analysis
        """
        ...
        if stats['mixer_count'] > 0:
            logger.warning(
                f"🚨 Mixer interactions detected in report: {stats['mixer_count']} transactions, "
                f"{stats['sanctioned_mixer_count']} with sanctioned entities"
            )
        ...
        # Quick final calls with actual compliance data
        ...
        # Save to database with CORRECT relative paths
        async with AsyncSessionLocal() as session:
